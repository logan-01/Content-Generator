...
function meme() {
  const meme = document.querySelector("#meme-btn");
  const memeData = randomData("memes");
  const memeImages = document.createElement("img");
  memeImages.setAttribute("src", memeData);
  clearAll();
...
function joke() {
  const joke = document.querySelector("#joke-btn");
  const jokeData = randomData("jokes");
...
function reveal() {
  ...
  if (question && answer.hidden) {
    ...
    alert("Answer has been already revealed");
  } else {
    alert("There has no riddle present");
  }
}
